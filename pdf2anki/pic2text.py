...
# The OpenRouter HTTP transport (per-process Session cache, SSE upgrade,
# idle/wall/char guards) lives in openrouter_transport.py, shared with
# text2anki.llm_helper. The names below are re-bound here because this
# module's callers reference them as module globals and the test suite
# patches them at `pdf2anki.pic2text.*`. The HTTP_* guard constants are
# deliberately NOT re-bound: a patched by-value copy here would silently
# have no effect on the transport -- patch them (and _get_session, for
# real-transport tests) on pdf2anki.openrouter_transport instead.
